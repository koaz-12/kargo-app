            ...
            <div className="flex bg-transparent relative">
                ...
                {onSelect && (
                    <div className="absolute top-0 bottom-0 left-0 w-10 flex items-center justify-center z-10" onClick={(e) => { e.stopPropagation(); onSelect(p.id, !isSelected); }}>
                        <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors cursor-pointer ${isSelected ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-slate-300 hover:border-blue-400'}`}>
                            {isSelected && <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3"><polyline points="20 6 9 17 4 12"></polyline></svg>}
                        </div>
                    </div>
                )}
